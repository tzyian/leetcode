...
Output: [6.00000,0.50000,-1.00000,1.00000,-1.00000
    ...
    fun calcEquation(equations: List<List<String>>, values: DoubleArray, queries: List<List<String>>): DoubleArray {
        ...
        for (ind in queries.indices) {
            ...
            val value = DFS(graph, firstVar, secondVar, 1.0, mutableSetOf())
            result[ind] = value
        }
    ...
    private fun DFS(graph: MutableMap<String, MutableMap<String, Double>>, firstVar: String, secondVar: String, div: Double, checked: MutableSet<String>): Double {
        ...
        for (str in graph[firstVar]!!.keys) {
            if (str !in checked) {
                checked.add(str)
                val check = DFS(graph, str, secondVar, div, checked)
                if (check != -1.0) {
                    return div * graph[firstVar]?.get(str)!! * check
                }
            }
        }
